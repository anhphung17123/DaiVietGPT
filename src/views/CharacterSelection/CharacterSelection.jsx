      ...
      <div className="characterSelection">
        ...
        {characters.map((character) => (
          <img
            ...
            className={`characterSelection-background ${
              selectedCharacter?.name === character.name ? "characterSelection-background__selected" : ""
            }`}
        ...
        {characters.map((character) => (
          <div key={`content-${character.id}`}>
            <div
              className={`characterSelection-description ${
                selectedCharacter?.name === character.name ? "characterSelection-description__selected" : ""
              }`}
            >
            ...
            <div
              className={`characterSelection-action ${
                selectedCharacter?.name === character.name ? "characterSelection-action--active" : ""
              }`}
            >
              <button
                className="characterSelection-action-button"
                onClick={() =>
                  character.isLocked === 0 ? handleSelectCharacter(character.id) : handleShoppingCharacter(character.id)
                }
                aria-label={character.isLocked === 0 ? `Chọn ${character.name}` : `Mua ${character.name}`}
              >
                <FontAwesomeIcon icon={character.isLocked === 0 ? faArrowRight : faCartShopping} />
              </button>
            </div>
          </div>
        ))}
          ...
          {characters.map((character) => (
            <li
              ...
              className={`characterSelection-item ${
                selectedCharacter?.name === character.name ? "characterSelection-item__selected" : ""
              }`}
